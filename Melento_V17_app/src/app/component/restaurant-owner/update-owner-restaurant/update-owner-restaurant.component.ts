...
@Component({
  selector: 'app-update-owner-restaurant',
  templateUrl: './update-owner-restaurant.component.html',
  styleUrls: ['./update-owner-restaurant.component.scss']
})
export class UpdateOwnerRestaurantComponent {
  ...
  selectArrRestaurants:Restaurant[]=[]
    ...
    this.restaurantService.getRestaurants().subscribe(data=>{
      ...
      for(let i=0;i<this.arrRestaurants.length;i++){
        if(this.arrRestaurants[i].r_owner_id == user_id){
          this.selectArrRestaurants.push(this.arrRestaurants[i]);
        }
      }
